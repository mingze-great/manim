...
后续 `v2.0` 及其后续版本，统一采用下面这套执行模型：

1. **以远程基线分支为起点**
   - 当前 `v2.0` 基线分支：`feature/v2-ui-mobile-ux-recover-3003`
   - 后续所有优化都必须从 `origin/<baseline-branch>` 新建分支
   - 不允许从本地脏工作区直接继续分叉开发

2. **每个优化分支都必须绑定独立 worktree**
   - 所有开发都在独立 worktree 中进行
   - 不允许多个优化长期共用同一个主工作区
   - 目的：避免本地历史改动、实验代码、临时文件污染当前优化分支

3. **分支创建后立即推送远程**
   - 先固定基线，再开始开发
   - 保证任何时刻都可以从远程恢复当前工作分支

4. **所有修改必须先提交、再推送、再部署**
   - 不允许依赖“本地未推送代码”部署
   - 不允许依赖“服务器热修但仓库没有”的状态

5. **部署必须来源于远程分支**
   - 标准做法：
   - 服务器 `git fetch`
   - 服务器 `git checkout <branch>`
   - 服务器 `git reset --hard origin/<branch>`
   - 然后再执行部署脚本
...
8. **只有验证通过的远程分支，才允许作为下一轮优化基线**
   - 即：
   - 已提交
   - 已推送
   - 已从远程分支完成部署验证

9. **后续所有 v2 新需求默认继承这套流程**
   - 远程基线分支 -> 新分支 -> 独立 worktree -> 提交 -> 推远程 -> 从远程部署
...
- `feature/v2-script-layout-optimization`
- `fix/v2-template-preview-stability`
- `feature/v2-admin-mobile-cards`

### 1.2.3 V2 worktree 命名建议

推荐为每个优化分支创建可识别的独立目录，例如：

- `E:\ai\wt_v2_script_layout`
- `E:\ai\wt_v2_template_preview`
- `E:\ai\wt_v2_admin_mobile`

原则：

- 一个目标分支，一个独立 worktree
- 不混用，不复用脏目录，不在主工作区反复切分支开发
