...
## 平台闭环开发约束
- 平台产品需求必须落到平台 UI/API 的真实链路上，不能只在本地脚本中实现“看起来可用”的旁路能力。
- 每次部署、同步远程、上下文压缩或交接前，必须更新 `PROJECT_STATE.md`，写明当前分支、提交、部署状态、验证结果和下一步。
- 新增用户入口时要优先接入现有登录态、权限、套餐限制和路由体系，避免创建脱离平台的临时页面或临时接口。
- 3004 是本轮新功能验证环境；除非用户明确要求，不修改、不重启、不覆盖 3003。
- AI 助手回答面向普通用户和合作者，不暴露服务器路径、密钥、root 账号、内部部署细节或管理员敏感操作。
