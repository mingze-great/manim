...
def detect_watermark_bbox(image: Image.Image):
    width, height = image.size
    left_bound = int(width * 0.72)
    top_bound = int(height * 0.78)
    pixels = image.load()
    left = width
    top = height
    right = -1
...
def remove_background_to_alpha(image: Image.Image):
    ...
    offset_x = (rgba.width - cropped.width) // 2
    offset_y = (rgba.height - cropped.height) // 2
    canvas.alpha_composite(cropped, (offset_x, offset_y))
    return canvas


def build_clean_alpha_material(source_path: Path, target_path: Path):
    image = Image.open(source_path).convert("RGB")
    watermark_bbox = detect_watermark_bbox(image)
    if watermark_bbox:
        draw = ImageDraw.Draw(image)
        draw.rectangle(watermark_bbox, fill=(255, 255, 255))
    cleaned = remove_background_to_alpha(image)
    cleaned.save(target_path, format="PNG")


def main():
    source_dir = Path(sys.argv[1]) if len(sys.argv) > 1 else Path(r"E:\ai\cankao\sucai_cropped")
    target_dir = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(r"E:\ai\cankao\sucai_clean_alpha")
    ...
    for entry in entries:
        ...
        build_clean_alpha_material(source_path, target_path)
        cloned = dict(entry)
