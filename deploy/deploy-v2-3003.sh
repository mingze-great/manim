...
NGINX_CONF_SRC="${NGINX_CONF_SRC:-$APP_ROOT/deploy/manim-v2-3003.conf}"
NGINX_CONF_DST="${NGINX_CONF_DST:-/etc/nginx/sites-available/manim-v2-3003.conf}"
BACKEND_ENV_SRC="${BACKEND_ENV_SRC:-$APP_ROOT/deploy/env.backend.3003.example}"
FRONTEND_ENV_SRC="${FRONTEND_ENV_SRC:-$APP_ROOT/deploy/env.frontend.3003.example}"
...
cd "$APP_ROOT"
chmod +x "$SYNC_TEMPLATE_VIDEOS_SCRIPT"
"$SYNC_TEMPLATE_VIDEOS_SCRIPT"

cp "$NGINX_CONF_SRC" "$NGINX_CONF_DST"
ln -sf "$NGINX_CONF_DST" /etc/nginx/sites-enabled/manim-v2-3003.conf
