        ...
        {templates.map((record) => {
          ...
          return (
            <Card key={record.id} size="small" style={{ borderRadius: '12px' }}>
              ...
              <div className="mt-3 text-xs text-gray-500 line-clamp-5 whitespace-pre-wrap">
                {(record.reference_code || record.code || '').slice(0, 220) || '暂无模板内容'}
                {((record.reference_code || record.code || '').length > 220) ? '...' : ''}
              </div>
              <div className="mt-3 flex flex-wrap gap-2">
