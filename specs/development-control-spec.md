...
5. 远程部署：需要同步服务器或重启服务。
6. 多需求并行：用户希望同时开发多个需求，并由 master agent 统一分发和验收。
...
- 多需求并行：必须读取 `rules/agent-orchestration.md` 和 `specs/master-agent-workflow-spec.md`，先拆分任务边界，再决定使用 subagent、thread、worktree 或串行执行。
...
6. 如果任务可并行，按 `rules/agent-orchestration.md` 分发给 worker agent。
7. 做最小可验证修改。
8. 运行本地检查。
9. 如果影响成片，生成或验证平台成片。
