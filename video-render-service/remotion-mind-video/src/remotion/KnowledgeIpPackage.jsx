  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {tabs.map((tab) => (
          <div key={tab} style={{
            ...
            background: tab === active.tab ? 'rgba(215,189,134,.34)' : 'transparent'
          }}>
            {tab}
          </div>
        ))}
      ...
      <div style={{
        ...
        <div style={{position: 'absolute', left: 22, top: 7, fontSize: 37, fontWeight: 1000, color: '#111'}}>《问题结构学》</div>
        <div style={{position: 'absolute', right: 24, top: 17, fontSize: 20, fontWeight: 700, color: '#222'}}>哲学知识分享 · 认知提升</div>
        </div>
    </div>
  );
};
...
const MaterialScene = ({active, localFrame}) => {
  ...
  const materialSrc = active.materialSrc || active.assetSrc || active.videoSrc || active.media?.src;
  const MaterialClip = () => {
      ...
      <div style={{...common, background: '#111'}}>
        <OffthreadVideo
          ...
          style={{
            ...
            transform: `scale(${1.02 + enter * 0.035 + localFrame / segmentFrames * 0.035}) translate(${Math.sin(localFrame / 32) * 8}px, ${Math.cos(localFrame / 35) * 5}px)`
          }}
        />
        <div style={{
          position: 'absolute',
...
const SubtitleBand = ({caption}) => {
  ...
  const en = caption?.en || '';
  const lines = splitTwoLines(zh, 20);
  const charCount = lines.join('').length;
  const fontSize = lines.length > 2 ? 31 : charCount > 42 ? 36 : charCount > 30 ? 42 : 50;
  return (
  <div style={{
    position: 'absolute',
    top: 652,
...
export const KnowledgeIpPackage = ({
  ...
  return (
    <AbsoluteFill style={{fontFamily: 'PingFang SC, Microsoft YaHei, Arial, sans-serif', overflow: 'hidden', background: paper}}>
      ...
      <MaterialScene active={active} localFrame={localFrame} />
