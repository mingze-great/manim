...
@router.get("/{project_id}/explainer-generate")
async def generate_explainer_video_stream(
    project_id: int,
    current_user: Annotated[User, Depends(get_current_user)],
    db: Annotated[Session, Depends(get_db)]
):
    ...
    if not project:
        async def error_gen():
            yield f"data: {json.dumps({'type': 'error', 'content': 'Project not found'})}\n\n"
        return StreamingResponse(error_gen(), media_type="text/event-stream")
    if str(project.module_type or "manim") != "explainer":
        async def error_gen():
            yield f"data: {json.dumps({'type': 'error', 'content': '当前项目不是讲解型视频模块'})}\n\n"
        return StreamingResponse(error_gen(), media_type="text/event-stream")

    task = Task(project_id=project_id, user_id=current_user_id, task_type="explainer_generate", status="pending", progress=0, log="")
    db.add(task)
    db.commit()
    db.refresh(task)
    ...
    async def event_generator():
        ...
        try:
            ...
            task_local.progress = 1
            project_local.status = "rendering"
            task_local.log = (task_local.log or "") + "开始讲解型视频生成\n"
            db_session.commit()
            ...
            def report(progress: int, message: str):
                loop.call_soon_threadsafe(progress_queue.put_nowait, {"type": "progress", "progress": progress, "content": message})
            ...
            task_local.video_url = video_url
            task_local.error_message = None
            task_local.log = (task_local.log or "") + "讲解型视频生成完成\n"
            db_session.commit()
            yield f"data: {json.dumps({'type': 'success', 'content': '讲解型视频生成完成', 'video_url': video_url})}\n\n"
        except Exception as exc:
            task_local = db_session.query(Task).filter(Task.id == task.id).first()
            project_local = db_session.query(Project).filter(Project.id == project_id).first()
            if task_local:
...
@router.get("/{project_id}/explainer-compose")
async def compose_explainer_video_stream(
    project_id: int,
    current_user: Annotated[User, Depends(get_current_user)],
    db: Annotated[Session, Depends(get_db)]
):
    ...
    task = Task(project_id=project_id, user_id=current_user_id, task_type="explainer_compose", status="pending", progress=0, log="")
    db.add(task)
    db.commit()
    db.refresh(task)
    ...
    async def event_generator():
        ...
        try:
            ...
            task_local.progress = 1
            project_local.status = "rendering"
            task_local.log = (task_local.log or "") + "开始讲解型视频合成\n"
            db_session.commit()
            progress_queue: asyncio.Queue[dict] = asyncio.Queue()
            loop = asyncio.get_running_loop()
            ...
            task_local.video_url = video_url
            task_local.error_message = None
            task_local.log = (task_local.log or "") + "讲解型视频合成完成\n"
            db_session.commit()
            yield f"data: {json.dumps({'type': 'success', 'content': '讲解型视频合成完成', 'video_url': video_url})}\n\n"
        except Exception as exc:
            task_local = db_session.query(Task).filter(Task.id == task.id).first()
            project_local = db_session.query(Project).filter(Project.id == project_id).first()
            if task_local:
