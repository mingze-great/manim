...
class Dynamic_HUD_Review(Scene):
    def construct(self):
        ...
        # --- 3. 动画流 (严禁修改) ---
        title_text = Text(INTRO_TITLE, font=CHINESE_FONT, color=COLORS[0], weight=BOLD).scale(1.5)
        ...
        for i, (title_str, desc_str, v_type) in enumerate(models):
            ...
            # 左侧文字排版
            title = Text(title_str, font=CHINESE_FONT, color=current_color, weight=BOLD).scale(1.2)
            desc = Text(desc_str, font=CHINESE_FONT, color=WHITE, line_spacing=1.5).scale(0.8)
        ...
        # 结尾
        outro = Text(OUTRO_TEXT, font=CHINESE_FONT, color=WHITE, line_spacing=1.5).scale(1.2)
